...
def _pack_protobuf_into_graph_spec(tf_block_proto):
...
def _pack_concatenated_bindings(old_bindings, tensor_name_maps):
  ...
  nonempty_remapped_bindings = [x for x in remapped_bindings if x is not None]
  if not nonempty_remapped_bindings:
    return None
  if len(nonempty_remapped_bindings) == 1:
    return nonempty_remapped_bindings[0]
  return pb.TensorFlow.Binding(
      tuple=pb.TensorFlow.NamedTupleBinding(element=nonempty_remapped_bindings))
...
def concatenate_tensorflow_blocks(tf_comp_list):
  ...
  (merged_graph, init_op_name, parameter_name_maps,
   result_name_maps) = graph_merge.concatenate_inputs_and_outputs(
       [_pack_protobuf_into_graph_spec(x) for x in tf_proto_list])
